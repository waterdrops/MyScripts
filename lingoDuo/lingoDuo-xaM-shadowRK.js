/**
 * @name Duolingo Max Unlock (Shadowrocket)
 * @author Takagivegeta
 * @description 采用通用 Env 兼容层重写，模仿 Maasea 风格，确保 Shadowrocket 兼容性。
 */

const $ = new Env("Duolingo Max");
...
(async () => {
    try {
        ...
        // 检查数据结构是否符合预期 (Duolingo 的 batch 接口)
        if (obj.responses && obj.responses.length > 0 && obj.responses[0].body) {
            
            $.log("🔍 检测到 batch 数据，开始解析...");
            
            // ⚠️ 关键点：多邻国的内部 body 是字符串化的 JSON，需要二次解析
            let userStr = obj.responses[0].body;
            let userData = JSON.parse(userStr);

            const timestamp = Math.floor(Date.now() / 1000);

            // 1. 注入订阅信息 (模拟 Gold/Max 购买)
            ...
            userData.shopItems.push({
                ...
                "purchaseDate": timestamp - 10000,
                "purchasePrice": 0,
                "subscriptionInfo": {
                    "expectedExpiration": timestamp + 31536000, // 1年有效期
                    "productId": "com.duolingo.DuolingoMobile.subscription.Gold.TwelveMonth.24Q2Max.168",
                    "renewer": "APPLE",
                    "renewing": true,
                    "tier": "twelve_month",
                    "type": "gold" // 虽然叫 gold，但在 Max 开启 flags 后会显示 Max
                }
            });

            // 2. 强制修改用户等级
            userData.subscriberLevel = "GOLD";
            userData.has_plus = true;

            // 3. 开启所有功能标志 (Feature Flags)
            ...
            const features = [
                ...
                "has_item_max_subscription" // 关键：Max 权限
            ];
            
            features.forEach(f => {
                userData.trackingProperties[f] = true;
            });
            ...
            $.log("⚠️ 未找到预期的 responses 结构，原样返回");
            $.done({});
        }
...
// --- 以下是通用兼容层 (Env), 类似于 Maasea 脚本所使用的 ---
function Env(t, e) {
  "undefined" != typeof process && JSON.stringify(process.env).indexOf("GITHUB") > -1 && process.exit(0);
  class s {
    constructor(t) { this.env = t }
    send(t, e) { t = "json" === e ? JSON.stringify(t) : t, this.env.isLoon() ? $notification.post(t.title, t.subtitle, t.body) : this.env.isQuanX() ? $notify(t.title, t.subtitle, t.body) : this.env.isSurge() ? $notification.post(t.title, t.subtitle, t.body) : $notification.post(t.title, t.subtitle, t.body) }
  }
  return new class {
    constructor(t, e) {
    ...
    runScript(t, e) {
      return new Promise(s => {
        ...
        const [o, h] = i.split("@"), n = { url: `http://${h}/v1/scripting/evaluate`, body: { script_text: t, mock_type: "cron", timeout: r }, headers: { "X-Key": o, "Content-Type": "application/json" } };
        this.post(n, (t, i, r) => s(r))
      })
    }
    loaddata() {
      if (!this.isNode()) return {};
      {
        ...
        if (!s && !i) return {};
        { const i = s ? t : e; try { return JSON.parse(this.fs.readFileSync(i)) } catch { return {} } }
      }
    }
    writedata() {
      if (this.isNode()) {
    ...
    post(t, e = (() => { })) {
      ...
      else if (this.isQuanX()) t.method = "POST", this.isNeedRewrite && (t.opts = t.opts || {}, Object.assign(t.opts, { hints: !1 })), $task.fetch(t).then(t => { const { statusCode: s, statusCode: i, headers: r, body: o } = t; e(null, { status: s, statusCode: i, headers: r, body: o }, o) }, t => e(t));
      else if (this.isNode()) { this.initGotEnv(t); const { url: s, ...i } = t; this.got.post(s, i).then(t => { const { statusCode: s, statusCode: i, headers: r, body: o } = t; e(null, { status: s, statusCode: i, headers: r, body: o }, o) }, t => { const { message: s, response: i } = t; e(s, i, i && i.body) }) }
    }
    time(t, e = null) { const s = e ? new Date(e) : new Date; let i = { "M+": s.getMonth() + 1, "d+": s.getDate(), "H+": s.getHours(), "m+": s.getMinutes(), "s+": s.getSeconds(), "q+": Math.floor((s.getMonth() + 3) / 3), S: s.getMilliseconds() }; /(y+)/.test(t) && (t = t.replace(RegExp.$1, (s.getFullYear() + "").substr(4 - RegExp.$1.length))); for (const e in i) new RegExp("(" + e + ")").test(t) && (t = t.replace(RegExp.$1, 1 == RegExp.$1.length ? i[e] : ("00" + i[e]).substr(("" + i[e]).length))); return t }
    msg(e = t, s = "", i = "", r) {
      const o = t => {
        if (!t) return t;
    ...
    wait(t) { return new Promise(e => setTimeout(e, t)) }
      ...
      // ⚠️ 这里特别为 Shadowrocket 做了适配，小火箭有时不需要 $done 的参数包裹
      if(this.isShadowrocket()) $done(t)
    }
    
    // 增加一个 getter 方便读取 body
    get response() {
        if(this.isSurge() || this.isLoon() || this.isShadowrocket()) return $response;
        return {};
    }
  }(t, e)
}
